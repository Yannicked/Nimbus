...
def fix_file():
    ...
    # get_data_image signature might already have mut or not
    ...
    # 2. Rewrite get_data_image entirely to be safe
    get_data_image_regex = r'pub async fn get_data_image\(.*?Path\(\(mut ens_str, time\)\): Path<\(String, i64\)>,.*?State\(state\): State<Arc<AppState>>,.*?\).*?Result<impl IntoResponse, \(StatusCode, String\)> \{(?:.*?)\n\}'

    # Actually, simpler to just replace the whole functions.

    # 3. Helper to replace function body
    def replace_func(name, new_body):
        nonlocal content
        # This is a bit risky with regex, let's use a more robust way if possible.
        # Since I have the content, I'll look for the start and end.
        ...
        content = content[:open_brace_idx] + new_body + content[i-1:]

    # New bodies
    get_data_image_body = r''' {
    ...
    replace_func('get_data_image', get_data_image_body)
    replace_func('get_value', get_value_body)
    replace_func('get_timeseries', get_timeseries_body)
