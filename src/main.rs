...
struct AppState {
    ...
    png_cache: DashMap<(i32, i64), Vec<u8>>, // Key: (ensemble, time_seconds)
...
async fn get_map(
    Path((ens, time)): Path<(i32, i64)>,
    State(state): State<Arc<AppState>>,
) -> Result<impl IntoResponse, (StatusCode, String)> {
    // Check cache
    if let Some(cached_png) = state.png_cache.get(&(ens, time)) {
    ...
    let ens_idx = meta
        ...
        .iter()
        .position(|&e| e == ens)
        .ok_or((StatusCode::BAD_REQUEST, format!("Invalid ensemble: {}", ens)))?;
    ...
    // Read 2D slice from NetCDF
    let raw_slice = read_netcdf_slice(&file_path, ens_idx, time_idx).map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
    ...
    // Render PNG image
    let png_bytes = render_png(&projected_grid);

    // Cache results
    state.png_cache.insert((ens, time), png_bytes.clone());
...
async fn get_value(
    ...
    let ens_idx = meta
        ...
        .iter()
        .position(|&e| e == q.ens)
        .ok_or((StatusCode::BAD_REQUEST, format!("Invalid ensemble: {}", q.ens)))?;
    ...
    if val_raw == 65535 {
        Ok(axum::Json(HashMap::from([
            ("status".to_string(), serde_json::Value::String("no_rain".to_string())),
            ("value".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(0.0).unwrap())),
        ])))
    } else {
        let value = (val_raw as f64) * 0.01;
        Ok(axum::Json(HashMap::from([
            ("status".to_string(), serde_json::Value::String("ok".to_string())),
            ("value".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(value).unwrap())),
        ])))
    }
}
